...
function smartExcerpt(text, limit = 200) {
  ...
  const trimmed = text.substr(0, limit);
  const lastSentence = Math.max(
    trimmed.lastIndexOf(". "),
    trimmed.lastIndexOf("! "),
    trimmed.lastIndexOf("? ")
  );
  if (lastSentence > 50) return trimmed.substr(0, lastSentence + 1) + " …";
...
async function loadPolicyData() {
  ...
  try {
    const response = await fetch('data/policyNews.json', { cache: 'no-store' });
